...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn dynamic_color_sequence(&mut self, prefix: String, index: usize, terminator: &str) {
        ...
        let func: Box<dyn Fn(Rgb) -> String + Sync + Send + 'static> = Box::new(move |color| {
            format!(
    ...
    fn clipboard_load(&mut self, clipboard: u8, terminator: &str) {
        ...
        let func: Box<dyn Fn(&str) -> String + Send + Sync> = Box::new(move |text| {
    ...
    fn text_area_size_pixels(&mut self) {
        let func: Box<dyn Fn(WindowSize) -> String + Sync + Send + 'static> = Box::new(move |window_size| {
        ...
        self.event_proxy.send_event(Event::TextAreaSizeRequest(Arc::from(func)));
    }
